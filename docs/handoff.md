...
- 分支：`main`
- 本轮检查点提交：`feat: add third-party resource and character conversion engines`（用 `git log -1` 取得实际 hash）。
- 当前工作树在提交后应为空；未 push。
...
1. 用户尚未批准下一个模板。上次给出的建议是基德“专属卡（`story`）”：规范字段为名称、
   触发、游戏效果`描述`、`风味描述`；基德精确映射，dhsheet/ZZZ 使用开放扩展记录，RinkCX 拒绝。
   先取得用户确认再实现。
2. 继续坚持“一次一个模板”：先从固定 revision 源码确认原生结构，再给用户字段与损失裁定，
   获批后实现 Template、Adapter、目标引擎测试、文档并运行 `npm run verify`。
3. 基德尚未裁定的扩展类型包括 `calamity`、`ingredient`、`meal`、`transformation`、`material`、
   `vehicle`、`madness`、`clue`、`prophecy`、`question`、`quest`、`wheelchair`、`anomaly`、
   `stronghold`、`landmark` 等；不得仅因字段相近就映射到现有 Template。
4. 所有资源模板完成后，再回到人物卡临时 Profile，逐字段对照正式 Character Save Contract；
