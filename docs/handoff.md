...
- 顺序属于当前浏览器的界面偏好，分别存入 localStorage 的 `pbdh.creator.tabs.resources` 与 `pbdh.creator.tabs.tabletops`；不写入资源包、GM 桌面文档、Contract 或云端 revision。Creator 资源标签使用 `[workspaceKey, resourceId]` 复合身份，因此可以跨资源包排序且不会因不同包内同名/同 ID 资源发生冲突。
...
- 新增 `tests/creator/tab-order.test.ts`，覆盖新建/关闭标签对已存顺序的协调、前后落点、跨资源包复合身份和 localStorage 读写去重。最新 `npm run verify` 通过：TypeScript 99 个文件、743 项测试；Python 141 项测试；依赖边界、Contract release、唯一视觉源、类型检查、Renderer 性能测量与 Platform 构建均通过。

### 当前卡面结论与下一步原型方式

- **武器和护甲卡面依然没有做好，当前版本绝不能视为视觉定稿或可接受方案。** 现有代码只是回退后的历史基线：武器仍有重复信息的 `weapon-summary`、微型画布和笨重的等权统计块；护甲正文同样存在信息少却构图松散、视觉重心失衡的问题。它们与已经相对稳定的种族、社群卡面仍有明显质量差距。
- 用户已经否定本模型此前完成的两轮重画；交给其他模型尝试的卡面结果同样很差，也没有形成任何获用户认可的视觉方向。不要从这些失败版本中挑一版继续微调，不要因为结构测试通过、没有溢出或风格色值一致，就宣称卡面已经做好。
- 下一步不直接修改正式 Weapon/Armor Renderer。先建立一个与生产代码隔离的**独立 HTML 原型页面**，在同一页面一次性展示多个真正不同的武器与护甲方案，让用户像“抽卡”一样批量查看、比较和筛选。目标是先抽到用户认为好看的视觉方向，再谈工程化落地。
- HTML 原型应使用同一组真实示例数据并排渲染，至少覆盖武器与护甲、短标题与长标题、无特性与短特性；每个候选方案必须有明确编号。不同候选应在信息层级、版式骨架、留白和视觉重心上真正拉开差异，不能只换圆角、边框、字号或把同一套三栏布局重新排列。
- 原型阶段可以复用已确认的家族色彩、字体气质和卡牌比例作为约束，但不得让现有 Renderer DOM/CSS 限制探索。用户选中明确方向之前，不更新视觉签名，不改 Contract/Template 字段，也不把任何候选接入正式 Creator。
- 用户选中候选后，再将该方案移植到正式 Renderer，补齐纯文字/图文/纯图片模式、固定与非固定比例、长文本拟合、溢出、真实 Creator 页面和控制台验收。`docs/card-face-redesign-guide.md` 只提供字段、文件结构和工程边界参考，不代表其中记录的任何旧视觉方案已获认可。
...
- 本地开发服务当前可用；Browser 插件已恢复，并完成了真实 Creator 指针拖拽、刷新持久化与控制台验收。
- 武器/护甲卡面已按要求回退到最近一次提交基线（HEAD），但该基线依然很差、尚未做好。此前本模型和其他模型给出的重画均未获用户认可；下一步必须先在独立 HTML 中批量生成多个差异足够大的原型，由用户选中方向后才允许改正式 Renderer。
