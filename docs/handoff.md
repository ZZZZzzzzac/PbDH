...
- v0.1.13（efbcbf1）已部署；Release run 34374267323、Deploy run 34374617243 成功，本地与 CI 完整 verify 通过。
- 子文件或文件夹可拖到所属资源包标题，或资源列表外的空白处，移动到该包根目录。文件夹内部放置仍由原树处理，避免冒泡重复移动。
- 拖放信封增加 workspaceKey，标题只接纳本包节点，外部空白按拖动来源定位，不误移动其他包的同 ID 资源。
- React 拖放事件回归覆盖标题内文字、包列表空白和树外容器三种落点，检查移动命令仅触发一次、parentId 和文件路径同步变为根目录。
