...
3. 直接在负责该表面的 React/HTML/CSS 中建立低保真结构和层级，使用受控 fixtures 在真实 App 外壳中评审布局、信息层级、状态表现和响应式行为。模板编辑区与规范卡面由精确模板版本自己的前端实现负责。
4. 把评审后的结构收敛为可交互浏览器原型；原型可以暂未接通后端，但不得伪装成纵切完成，也不得发明与正式 Contract 不同的临时数据模型。
5. 接通真实 Contract、业务逻辑、持久化和跨 App 边界，通过自动化测试、PRD 覆盖回读和真实浏览器交互/截图验收。源码是唯一视觉权威，不维护并行画布或生成映射。
6. 把实现中发现的错误假设回写到对应 PRD、ADR或覆盖表，再关闭本轮实现 Issue；不得只修代码而让正式设计决策继续表达旧行为。
...
每轮的正式范围、覆盖表、评审结论和验收证据归属对应 GitHub 实现 Issue。需要长期保留的视觉决策进入 ADR；实际视觉以负责该表面的前端源码和真实浏览器验收为准。
...
状态：已完成 L1 验收并关闭父 Issue #5；后续视觉调整直接在唯一前端源码中完成并用真实浏览器验收。
...
定义爱好者资源包的公共发现、规则标签筛选、稳定分享链接、安装与显式更新、公共媒体引用和作者自行取消公开。
...
阶段状态：已完成。首条纵切所需的 #8—#22 均已发布并完成 triage；实现完成的 L2 按验收结果关闭，其余父 Issue 继续开放。
...
5. `System Package 创作与资源兼容声明`（#12，已关闭）：建立首条纵切所需的最小 System Package 创作与运行时声明能力，产出一个真实 Daggerheart System Package，供 Player 验证按精确 Resource Template ID 与版本范围声明的原生路由兼容性；不把 Loader、CLI 或 Preview 实现组件升格为产品能力。
...
7. 为第一条纵切建立 PRD 覆盖表，并在真实前端原型中完成低保真结构、相关状态清单和人工布局评审；设计范围只覆盖该纵切实际经过的界面。
...
阶段状态：已完成。实现 Issue #38—#45 已全部完成并关闭；敌人与武器两条真实纵切均通过自动化、真实浏览器、云恢复和联合人工验收。
...
- 完成 `公共资源发现`、`出版生命周期`、`资源取得与 App 交接` 三个 L2。
- 完成包级/资源级搜索、稳定链接、从现有账号信息派生的作者页、取消发布/重新发布和响应式体验。
- Market 定位为爱好者公开分享资源包的目录，不建设审核、举报、版权处理、后台治理、人工精选或审计系统。
- 保持第一版免费、单作者、无社区功能、无历史版本下载、无 Market 直接上传或编辑资源，也不为未确认的未来功能预建数据表。
...
7. 作者取消公开后，已下载文件和既有有效 Cloud Document 仍按已接受规则恢复媒体。
8. 账号注销、文档永久删除和媒体引用释放。
