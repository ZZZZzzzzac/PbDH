...
- Player 桌面使用平台 `TabletopInstanceResourceCopy`，随同一 Character Save revision 原子保存，不建立独立玩家桌面文档。
- 云恢复时把自包含 Resource Copy 与媒体临时投影回 Sheet Runtime；复制人物或再次保存不依赖来源 `.pbres`。
- 本地 Repository、显式上云、outbox、冲突三动作、删除与 30 天回收站继续复用平台 Cloud Document Service。
- 原生文件交换使用 Character Save Contract 的 `.pbcha`，不读写旧 Sheet 内部 Character Data JSON。
