...
`presentation.fixedRatio` continues to describe the text composition. In split mode the portrait extends that base surface, so the effective outer surface is not constrained to `63:88`. The text composition is pushed down without image-triggered text fitting. If authored text no longer fits its template-owned text area, the author must revise the crop or content.
...
- Shared surface hosts treat split presentation as height-bearing even when `fixedRatio` is true, so Creator, Player, Market and Tabletop do not clip the extended Renderer.
