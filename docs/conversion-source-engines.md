...
- 该 Template 已注册完整 Core、作者布局与共享 Renderer。
...
- RinkCX 不支持职业资源，转换保持拒绝。

“子职业”使用 `子职业@1.0.0`，裁定如下：
...
- dhsheet 的基石、专精、大师与规范等级双向映射；ZZZ 的名称等级后缀在导入时归一化，
  导出时恢复；
- RinkCX 不支持子职业资源，转换保持拒绝。

“罗德岛子职”是罗德岛旅记（tttri）官方资源包自有的子职模板 `罗德岛子职@1.0.0`，与 daggerheart 的`子职业`互不共用：

- 规范字段为名称、类型、主职、等级（预备、正式、资深、精英X、精英Y）、阶段（T1–T4Y）、施法属性、
  推荐次领域、武器原型、子职提升，以及按覆盖语义自带当前生效的子职特性、职业特性与希望特性
  （未变化的框留空表示不改动），另有结构化特性数组与简介；
- 它只服务 tttri 的 `subclasses`（干员）资源库；子职业选择器与填充依赖按字段名消费，不读取模板 id；
- 外部格式转换仍只产出 daggerheart 语义的 `子职业@1.0.0`；`罗德岛子职`仅在 pbres 包与 dhsheet 之间
  往返时按 `subclass` 类别处理；
- 该 Template 已注册完整 Core、作者布局与共享 Renderer。
...
- RinkCX 不支持社群资源，转换保持拒绝。

“领域卡”使用 `领域卡@1.0.0`，裁定如下：
...
- dhsheet 使用 `variant` 自定义类型，ZZZ 使用开放记录，两者完整携带自由模板字段；RinkCX
  不支持自由资源，转换保持拒绝；
- dhsheet `variant` 的`子类别`和`简略信息`属于卡面分类标签，映射为自由字段；对象型
  `简略信息.itemN`的非空值映射为空值自由字段的字段名，以纯标签显示，不得序列化成 JSON
  自由特性；字段名冲突时才退回编号自由字段；
