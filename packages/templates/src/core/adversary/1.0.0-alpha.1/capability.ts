...
export const adversaryTemplate = deepFreeze<TemplateCoreCapability<AdversaryData>>({
  ...
  proposeResourceId(data) {
    const candidate = normalizeSearchPart(data.名称 || data.原文 || "未命名敌人");
    return candidate.normalize("NFC").replaceAll("/", "-");
  },
  project(data) {
    const title = normalizeSearchPart(data.名称 || data.原文 || "未命名敌人");
    const summary = normalizeSearchPart(data.简介);
    const featureText = data.特性.flatMap((feature) => [
      feature.名称,
      feature.原名,
      feature.类型,
      feature.特性描述,
  ...
  tabletop: {
    stateSchema: {
      ...
      properties: {
        currentHp: { type: "string" },
        currentStress: { type: "string" },
        focused: { type: "string", enum: ["true", "false"] },
        notes: { type: "string" },
      },
