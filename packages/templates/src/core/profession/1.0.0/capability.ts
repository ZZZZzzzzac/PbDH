...
export type ProfessionData = {
  名称: string; 描述: string; 领域: string[]; 生命点: string; 闪避值: string; 职业物品: string;
  希望特性: string; 职业特性: string; 推荐初始属性: Record<string, string>; 推荐初始武器: string;
  推荐初始护甲: string; 背景问题: string[]; 关系问题: string[]; 施法属性: string;
};

const defaultData: ProfessionData = {
  名称: "", 描述: "", 领域: [], 生命点: "", 闪避值: "", 职业物品: "", 希望特性: "",
...
export const professionTemplate = deepFreeze<TemplateCoreCapability<ProfessionData>>({
  ...
  project(data) {
    ...
    const searchText = Object.values(data).flatMap((value) => Array.isArray(value)
      ? value
