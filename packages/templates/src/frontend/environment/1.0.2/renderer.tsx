...
import { environmentTemplate, type EnvironmentData } from "../../../core/environment/1.0.2/capability.ts";
  ...
  return <section className={`environment-feature${expanded ? " is-expanded" : ""}`}>
    ...
    >
      <span className="environment-feature-primary">
        ...
        <span className="environment-feature-type">{feature.特性类型}</span>
      </span>
    </button>
    {expanded && feature.特性描述 ? <><span className="environment-feature-separator">：</span><span className="environment-feature-copy"><RestrictedMarkdown inline value={feature.特性描述} /></span></> : null}
...
export const environmentRendererRevision: RendererRevisionCapability<EnvironmentData, EnvironmentRuntimeState, ReactNode> = {
  ...
  render({ data, presentation, assets, attribution }) {
    ...
    return <EnvironmentCardFrame fixedRatio={fixedSurface} splitFixed={splitFixed} imageKey={portrait}>{(cardRef) => <article ref={cardRef} className={["environment-card", `is-${presentation.mode}`, fixedSurface ? "" : "is-fluid", splitFixed ? "has-fixed-base" : "", portrait ? "has-portrait" : ""].filter(Boolean).join(" ")} data-renderer-revision="environment-card-r4">
