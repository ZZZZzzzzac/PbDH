...
import { environmentTemplate, type EnvironmentData } from "../../../core/environment/1.1.1/capability.ts";
...
.environment-scene{display:grid;grid-template-columns:76px minmax(0,1fr);flex:none;background:#efe3cf;border-bottom:1px solid #d4b78d}.environment-difficulty{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:18px 8px;background:var(--oxblood);color:#fff4df}.environment-difficulty-label{font:800 12px/1.15 "Noto Sans SC",sans-serif;letter-spacing:.14em;writing-mode:vertical-rl}.environment-difficulty b{margin:12px 0;color:#fff4df;font:900 42px/.9 Georgia,"Noto Serif SC",serif}.environment-record{margin:0;padding:6px 18px}.environment-record>div+div{margin-top:0;padding-top:6px;border-top:1px solid #c6af8d}.environment-record dt{margin:0 0 5px;color:var(--oxblood);font:800 12px/1.2 "Noto Sans SC",sans-serif;letter-spacing:.08em}.environment-record dd{margin:0;white-space:pre-wrap;color:var(--ink);font:450 var(--environment-content-font-size,15px)/1.45 "Noto Sans SC",sans-serif}.environment-record .environment-intro{font-weight:650}
.environment-features-wrap{padding:12px 11px 14px}.environment-feature-heading{display:flex;align-items:center;gap:8px;margin:0 0 7px;color:var(--oxblood);font:800 13px/1 "Noto Sans SC",sans-serif;letter-spacing:.08em}.environment-feature-heading::after{content:"";height:2px;flex:1;background:var(--brass)}.environment-features{min-height:0;flex:1;overflow:hidden;display:flex;flex-direction:column;gap:6px}.environment-card.is-fluid .environment-features{flex:none;overflow:visible}.environment-feature{position:relative;padding:6px;display:block;background:#f7ebd6;border:1px solid #d4b78d;border-radius:4px}.environment-feature:not(.is-expanded){display:block}.environment-feature-toggle{min-width:0;display:inline;margin:0;padding:0;border:0;background:transparent;color:var(--oxblood);text-align:left;cursor:pointer}.environment-feature-toggle::before{content:"+";display:inline-block;width:.8em;margin-right:6px;color:#87504b;font:700 16px/1 "Noto Sans SC",sans-serif}.environment-feature.is-expanded .environment-feature-toggle::before{content:"−"}.environment-feature:not(.is-expanded) .environment-feature-toggle{height:24px;overflow:hidden}.environment-feature-primary{min-width:0;display:inline}.environment-feature:not(.is-expanded) .environment-feature-primary{display:inline;white-space:nowrap;overflow:hidden}.environment-feature-name{margin:0 7px 0 0;color:var(--oxblood);font:800 calc(var(--environment-content-font-size,15px) + 3px)/1.2 "Noto Sans SC",sans-serif}.environment-feature:not(.is-expanded) .environment-feature-name{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.environment-feature-type{display:inline;margin:0 1px 0 0;color:#87504b;font:650 var(--environment-content-font-size,15px)/1.2 "Noto Sans SC",sans-serif}.environment-feature:not(.is-expanded) .environment-feature-type{flex:none;margin-top:0}.environment-feature-toggle small{display:inline;margin-right:7px;color:#5e4637;font:650 max(8px,calc(var(--environment-content-font-size,15px) - 3px))/1.2 "Noto Sans SC",sans-serif}.environment-feature-toggle:focus-visible{outline:2px solid var(--brass);outline-offset:2px}.environment-feature-separator{color:var(--ink);font:450 var(--environment-content-font-size,15px)/1.35 "Noto Sans SC",sans-serif}.environment-feature-copy{display:inline;color:var(--ink);font:450 var(--environment-content-font-size,15px)/1.35 "Noto Sans SC",sans-serif}.environment-feature-copy>[data-restricted-markdown]{display:inline;margin:0}.environment-feature-copy p{margin:0}.environment-question{grid-column:1/-1;margin:0!important;padding-top:6px;border-top:1px solid #d4b78d;color:#5e4637!important;font-size:max(8px,calc(var(--environment-content-font-size,15px) - 2px));line-height:1.4;font-style:italic!important}.environment-question strong{color:var(--oxblood);font-style:normal}.environment-card>.pbdh-card-footer{color:#725747;background:var(--bone);border-top:1px solid #d4b78d}.environment-card.is-image>.pbdh-card-footer{display:none}
...
.environment-feature-copy > [data-restricted-markdown] > p:first-child{display:inline}.environment-feature :is(ul,ol){display:block;margin:.25em 0;padding-inline-start:1.5em}.environment-feature ul{list-style-type:disc}.environment-feature ol{list-style-type:decimal}.environment-feature li{display:list-item}
...
function EnvironmentFeature({ feature }: { feature: EnvironmentData["特性"][number] }) {
  const [expanded, setExpanded] = useState(true);
  return <section className={`environment-feature${expanded ? " is-expanded" : ""}`}>
    <button
      type="button"
      className="environment-feature-toggle"
    ...
    >
      <span className="environment-feature-primary">
        <span className="environment-feature-name">{feature.特性名称 || "未命名特性"}</span>
        {expanded && feature.特性原文?.trim() ? <small>{feature.特性原文}</small> : null}
        <span className="environment-feature-type">{feature.特性类型}{expanded && feature.特性描述 ? <span className="environment-feature-separator">：</span> : null}</span>
      </span>
    </button>
    {expanded && feature.特性描述 ? <div className="environment-feature-copy"><RestrictedMarkdown value={feature.特性描述} /></div> : null}
    {expanded && feature.引导问题 ? <div className="environment-question"><RestrictedMarkdown value={feature.引导问题} /></div> : null}
  </section>;
}
export const environmentRendererRevision: RendererRevisionCapability<EnvironmentData, EnvironmentRuntimeState, ReactNode> = {
  revision: "environment-card-r6",
  templateId: "环境",
  ...
  render({ data, presentation, assets, attribution }) {
    ...
    return <EnvironmentCardFrame fixedRatio={fixedSurface} splitFixed={splitFixed} imageKey={portrait}>{(cardRef) => <article ref={cardRef} className={["environment-card", `is-${presentation.mode}`, fixedSurface ? "" : "is-fluid", splitFixed ? "has-fixed-base" : "", portrait ? "has-portrait" : ""].filter(Boolean).join(" ")} data-renderer-revision="environment-card-r6">
      ...
      <div className="environment-body">
        <section className="environment-scene" aria-label="环境资料">
          <div className="environment-difficulty"><span className="environment-difficulty-label">难度</span><b>{data.难度 || "—"}</b></div>
        ...
        <div className="environment-features-wrap"><h2 className="environment-feature-heading">环境特性 / FEATURES</h2><TextFitContainer className="environment-features" contentKey={JSON.stringify(data.特性)} enabled={fixedSurface} cssVariable="--environment-content-font-size">{data.特性.map((feature, index) => <EnvironmentFeature
          feature={feature}
          key={`${feature.特性名称}:${index}`}
        />)}</TextFitContainer></div>
      </div>
