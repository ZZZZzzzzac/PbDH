...
import { environmentTemplate, type EnvironmentData } from "../../../core/environment/1.0.3/capability.ts";
...
function EnvironmentFeature({ feature }: { feature: EnvironmentData["特性"][number] }) {
  ...
  return <section className={`environment-feature${expanded ? " is-expanded" : ""}`}>
    ...
    {expanded && feature.特性描述 ? <><span className="environment-feature-copy"><RestrictedMarkdown inline value={feature.特性描述} /></span></> : null}
    {expanded && feature.引导问题 ? <p className="environment-question"><RestrictedMarkdown inline value={feature.引导问题} /></p> : null}
  </section>;
}
export const environmentRendererRevision: RendererRevisionCapability<EnvironmentData, EnvironmentRuntimeState, ReactNode> = {
  revision: "environment-card-r5",
  templateId: "环境",
  ...
  render({ data, presentation, assets, attribution }) {
    ...
    return <EnvironmentCardFrame fixedRatio={fixedSurface} splitFixed={splitFixed} imageKey={portrait}>{(cardRef) => <article ref={cardRef} className={["environment-card", `is-${presentation.mode}`, fixedSurface ? "" : "is-fluid", splitFixed ? "has-fixed-base" : "", portrait ? "has-portrait" : ""].filter(Boolean).join(" ")} data-renderer-revision="environment-card-r5">
