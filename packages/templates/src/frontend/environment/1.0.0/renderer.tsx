...
import { environmentTemplate, type EnvironmentData } from "../../../core/environment/1.0.0/capability.ts";
...
export const environmentRendererRevision: RendererRevisionCapability<EnvironmentData, EnvironmentRuntimeState, ReactNode> = {
  ...
  render({ data, presentation, assets, attribution }) {
    ...
    return <EnvironmentCardFrame fixedRatio={presentation.fixedRatio}>{(cardRef) => <article ref={cardRef} className={["environment-card", `is-${presentation.mode}`, presentation.fixedRatio ? "" : "is-fluid"].filter(Boolean).join(" ")} data-renderer-revision="environment-card-r2">
