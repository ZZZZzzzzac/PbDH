    ...
    render({ data, presentation, assets }) {
      ...
      return <article className={`reference-card is-${presentation.mode}`} data-renderer-revision={input.revision} data-template-id={input.templateId}>
        ...
        <div className="reference-card-body">
          {model.summary && <p className="reference-card-summary">{model.summary}</p>}
          ...
          {model.sections?.filter((section) => section.body).map((section) => <section className="reference-card-section" key={section.title}><h2>{section.title}</h2><p>{section.body}</p></section>)}
          {model.flavor && <p className="reference-card-flavor">{model.flavor}</p>}
        </div>
      </article>;
    },
  };
