...
export const domainRendererRevision = createReferenceCardRenderer<DomainData>({
  ...
  model: (data) => ({ title: data.名称 || "未命名领域卡", originalTitle: data.原文, kicker: data.类型 || "领域卡", meta: [data.领域, data.等级 ? `${data.等级}级` : "", data.属性].filter(Boolean), stats: data.回想 ? [{ label: "回想", value: data.回想 }] : [], sections: [{ title: "能力", body: data.描述 }], flavor: data.风味描述 }),
});
