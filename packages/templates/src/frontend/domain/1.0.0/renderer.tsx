...
export const domainRendererRevision = createReferenceCardRenderer<DomainData>({
  ...
  model: (data) => ({ title: data.名称 || "未命名领域卡", kicker: data.领域 || "领域卡", meta: [data.等级 ? `${data.等级}级` : "", data.属性].filter(Boolean), stats: data.回想 ? [{ label: "回想", value: data.回想 }] : [], sections: [{ title: "能力", body: data.描述 }], flavor: data.风味描述 }),
});
