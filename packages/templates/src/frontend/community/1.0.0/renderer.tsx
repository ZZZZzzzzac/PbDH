...
export const communityRendererRevision: RendererRevisionCapability<CommunityData, Record<string, string>, ReactNode> = {
  ...
  render({ data, presentation, assets, attribution }) {
    const cardClass = `community-card is-${presentation.mode}${presentation.fixedRatio ? "" : " is-fluid"}`;
    if (presentation.mode === "image") return <CommunityFrame fixedRatio={presentation.fixedRatio}>{(ref) => <article ref={ref} className={cardClass} data-renderer-revision="community-card-r1" data-template-id="社群"><div className="community-card-image">{assets.portrait ? <img src={assets.portrait} alt={data.名称} /> : <div className="community-card-missing">缺少主图</div>}</div></article>}</CommunityFrame>;
    return <CommunityFrame fixedRatio={presentation.fixedRatio}>{(ref) => <article ref={ref} className={cardClass} data-renderer-revision="community-card-r1" data-template-id="社群">
      ...
      <div className="community-card-body">{data.简介 ? <RestrictedMarkdown className="community-card-summary" value={data.简介} /> : null}<TextFitContainer className="community-card-sections" contentKey={JSON.stringify(data)} enabled={presentation.fixedRatio} minFontSizePx={11} maxFontSizePx={15} cssVariable="--community-font-size">{data.性格 ? <section className="community-card-section"><h2>性格</h2><RestrictedMarkdown value={data.性格} /></section> : null}{data.特性.特性描述 ? <section className="community-card-section"><h2><span>{data.特性.特性名称 || "社群特性"}</span>{data.特性.特性原文?.trim() ? <small>{data.特性.特性原文}</small> : null}</h2><RestrictedMarkdown value={data.特性.特性描述} /></section> : null}</TextFitContainer></div>
