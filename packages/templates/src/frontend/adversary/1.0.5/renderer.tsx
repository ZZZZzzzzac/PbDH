...
import { adversaryTemplate, type AdversaryData } from "../../../core/adversary/1.0.5/capability.ts";
...
function AdversaryFeature({ feature }: { feature: AdversaryData["特性"][number] }) {
  ...
  return <article className={`enemy-feature${expanded ? " is-expanded" : ""}`}>
    ...
    {expanded ? <><span className="enemy-feature-copy"><RestrictedMarkdown inline value={feature.特性描述} /></span></> : null}
  </article>;
}
...
> = {
  ...
  render({ data, state, assets, presentation, attribution, onStateCommand }) {
    ...
    if (mode === "image") {
      return <AdversaryCardFrame fixedRatio={presentation.fixedRatio}>{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="enemy-card-r6" data-presentation-mode={mode}>
    ...
    return <AdversaryCardFrame fixedRatio={fixedSurface} splitFixed={splitFixed} imageKey={portrait}>{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="enemy-card-r6" data-presentation-mode={mode}>
