...
import { adversaryTemplate, type AdversaryData } from "../../../core/adversary/1.0.4/capability.ts";
...
export const adversaryCardRenderSource = {
  ...
  implementation: "packages/templates/src/frontend/adversary/1.0.4/renderer.tsx",
...
> = {
  ...
  render({ data, state, assets, presentation, attribution, onStateCommand }) {
    ...
    if (mode === "image") {
      return <AdversaryCardFrame fixedRatio={presentation.fixedRatio}>{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="enemy-card-r5" data-presentation-mode={mode}>
    ...
    return <AdversaryCardFrame fixedRatio={fixedSurface} splitFixed={splitFixed} imageKey={portrait}>{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="enemy-card-r5" data-presentation-mode={mode}>
