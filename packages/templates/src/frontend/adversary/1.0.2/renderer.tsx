...
import { adversaryTemplate, type AdversaryData } from "../../../core/adversary/1.0.2/capability.ts";
...
.enemy-feature-toggle { width: 100%; min-width: 0; display: block; margin: 0; padding: 0; border: 0; background: transparent; color: var(--oxblood); text-align: left; cursor: pointer; }
...
.enemy-feature:not(.is-expanded) .enemy-feature-toggle { height: 24px; padding-right: 22px; overflow: hidden; }
.enemy-feature:not(.is-expanded) .enemy-feature-primary { min-width: 0; display: flex; align-items: baseline; gap: 8px; white-space: nowrap; overflow: hidden; }
.enemy-feature:not(.is-expanded) .enemy-feature-name { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.enemy-feature:not(.is-expanded) .enemy-feature-type { flex: none; }
...
.enemy-feature-name { min-width: 0; overflow-wrap: anywhere; font: 800 calc(var(--enemy-feature-font-size, 15px) + 4px)/1.2 "Noto Sans SC", sans-serif; }
.enemy-feature-type { color: #87504b; font: 650 var(--enemy-feature-font-size, 15px)/1.2 "Noto Sans SC", sans-serif; }
.enemy-feature-toggle small { display: block; margin-top: 1px; color: #5e4637; font: 650 max(8px, calc(var(--enemy-feature-font-size, 15px) - 3px))/1.2 "Noto Sans SC", sans-serif; }
...
function AdversaryFeature({ feature }: { feature: AdversaryData["特性"][number] }) {
  ...
  return <article className={`enemy-feature${expanded ? " is-expanded" : ""}`}>
    ...
    {expanded ? <RestrictedMarkdown value={feature.特性描述} /> : null}
  </article>;
}
...
> = {
  ...
  render({ data, state, assets, presentation, attribution, onStateCommand }) {
    ...
    if (mode === "image") {
      return <AdversaryCardFrame fixedRatio={presentation.fixedRatio}>{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="enemy-card-r3" data-presentation-mode={mode}>
    ...
    return <AdversaryCardFrame fixedRatio={fixedSurface} splitFixed={splitFixed} imageKey={portrait}>{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="enemy-card-r3" data-presentation-mode={mode}>
