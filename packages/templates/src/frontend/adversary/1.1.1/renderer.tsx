...
import { adversaryTemplate, type AdversaryData } from "../../../core/adversary/1.1.1/capability.ts";
...
export const adversaryCardRenderSource = {
  ...
  implementation: "packages/templates/src/frontend/adversary/1.0.5/renderer.tsx",
...
.enemy-heading h1 { width: 100%; min-width: 0; overflow: hidden; margin: 0; color: #fff4df; font: 800 var(--enemy-title-font-size, 28px)/1.2 "Noto Sans SC", sans-serif; white-space: nowrap; overflow-wrap: anywhere; }
...
.enemy-state-label { width: 58px; height: 26px; flex: none; display: grid; place-items: center; background: #21150f; color: #f8ddba; font: 700 13px/1.25 "Noto Sans SC", sans-serif; }
...
.enemy-feature-toggle { min-width: 0; display: inline; margin: 0; padding: 0; border: 0; background: transparent; color: var(--oxblood); text-align: left; cursor: pointer; }
.enemy-feature-toggle::before { content: "+"; display: inline-block; width: .8em; margin-right: 6px; color: #87504b; font: 700 16px/1 "Noto Sans SC", sans-serif; }
.enemy-feature.is-expanded .enemy-feature-toggle::before { content: "−"; }
...
.enemy-feature-copy p { min-width: 0; margin: 0; overflow: visible; }
.enemy-feature-copy > [data-restricted-markdown] > p:first-child { display: inline; }
.enemy-feature-copy :is(ul, ol) { display: block; margin: .25em 0; padding-inline-start: 1.5em; }
.enemy-feature-copy ul { list-style-type: disc; }
.enemy-feature-copy ol { list-style-type: decimal; }
.enemy-feature-copy li { display: list-item; }
  ...
  return <article className={`enemy-feature${expanded ? " is-expanded" : ""}`}>
    ...
    >
      <span className="enemy-feature-primary">
        ...
        <span className="enemy-feature-type">{feature.特性类型}{expanded ? <span className="enemy-feature-separator">：</span> : null}</span>
      </span>
    </button>
    {expanded ? <div className="enemy-feature-copy"><RestrictedMarkdown value={feature.特性描述} /></div> : null}
  </article>;
}
...
> = {
  ...
  render({ data, state, assets, presentation, attribution, onStateCommand }) {
    ...
    if (mode === "image") {
      return <AdversaryCardFrame fixedRatio={presentation.fixedRatio}>{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="enemy-card-r7" data-presentation-mode={mode}>
    ...
    return <AdversaryCardFrame fixedRatio={fixedSurface} splitFixed={splitFixed} imageKey={portrait}>{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="enemy-card-r7" data-presentation-mode={mode}>
      <div className="enemy-art">
        ...
        <header className={`enemy-heading${hasOriginalTitle ? " has-original-title" : ""}`}>
          <div className="enemy-heading-copy">
            <SingleLineTextFit contentKey={data.名称} minFontSizePx={10} maxFontSizePx={28} cssVariable="--enemy-title-font-size">{data.名称}</SingleLineTextFit>
