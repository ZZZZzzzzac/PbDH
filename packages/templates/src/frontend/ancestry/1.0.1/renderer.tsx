...
import { ancestryTemplate, type AncestryData } from "../../../core/ancestry/1.0.1/capability.ts";
...
.ancestry-card.is-split { height: auto; min-height: 0; overflow: visible; }
.ancestry-card.is-split .ancestry-art { min-height: 0; overflow: visible; border-bottom: 0; }
.ancestry-card.is-split .ancestry-art::before,
.ancestry-card.is-split .ancestry-art::after { display: none; }
.ancestry-card.is-split .ancestry-art img { position: static; width: 100%; height: auto; object-fit: contain; opacity: 1; }
.ancestry-card.is-split.has-portrait .ancestry-heading { position: absolute; inset: auto 0 0; background: linear-gradient(180deg, #1d131000 0%, #1d1310b8 48%, #1d1310f5 100%); }
.ancestry-card.is-split:not(.has-portrait) .ancestry-heading { background: #251a14; border-bottom: 3px solid #b88a57; }
.ancestry-card.is-split.has-fixed-base { height: var(--split-fixed-native-height); }
`;
...
function AncestryCardFrame({
  ...
  useLayoutEffect(() => {
    ...
    const updateHeight = () => setNativeHeight(splitFixed ? fixedAncestryNativeHeight + (image?.offsetHeight ?? 0) : card.offsetHeight);
  ...
  return <div
    ...
    style={{ height: `${ancestryFrameHeight(fixedRatio, nativeHeight)}px`, "--split-fixed-native-height": `${nativeHeight}px` } as CSSProperties}
...
> = {
  ...
  render({ data, presentation, assets, attribution }) {
    ...
    const cardClass = [
      "ancestry-card", `is-${mode}`, fixedSurface ? "" : "is-fluid", splitFixed ? "has-fixed-base" : "", portrait ? "has-portrait" : "",
    ].filter(Boolean).join(" ");
    ...
    if (mode === "image") {
      return <AncestryCardFrame fixedRatio={presentation.fixedRatio} fitContentKey="">{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="ancestry-card-r2" data-presentation-mode={mode}>
    ...
    return <AncestryCardFrame fixedRatio={fixedSurface} splitFixed={splitFixed} imageKey={portrait} fitContentKey={fixedSurface ? fitContentKey : ""}>{(cardRef, featureContainerRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="ancestry-card-r2" data-presentation-mode={mode}>
