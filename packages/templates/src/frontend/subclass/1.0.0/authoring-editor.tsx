...
export function SubclassAuthoringEditor({ data, onValue }: TemplateAuthoringEditorProps) {
  ...
  return <div className="template-owned-editor subclass-editor">
    ...
    <section className="subclass-basics">{field("名称")}<EditorInput label="英文" value={data.原文} onChange={(value) => onValue("原文", value)} />{field("类型")}{field("主职")}{field("等级", ["基础", "进阶", "精通"])}{field("施法属性")}</section>
    <section className="subclass-features"><header><h3>特性</h3><button type="button" onClick={() => onValue("特性", [...features, { ...emptyFeature }])}>＋ 新增</button></header>
      {features.map((feature, index) => <article className="subclass-feature" key={index}>
        ...
        {pendingDelete === index ? <div className="subclass-feature-confirm"><span>确认删除“{textValue(feature.名称)}”？</span><button type="button" onClick={() => setPendingDelete(null)}>取消</button><button type="button" onClick={() => { onValue("特性", features.filter((_, rowIndex) => rowIndex !== index)); setPendingDelete(null); }}>确认删除</button></div> : null}
      </article>)}
    </section>
    <section>
