import { freeFieldEntries, freeFixedFieldNames } from "../../../core/free/1.0.1/capability.ts";
...
export function FreeAuthoringEditor({ data, onValue, onData }: TemplateAuthoringEditorProps) {
  ...
  return <div className="template-owned-editor free-editor"><style>{standardEditorStyles + `.free-editor .free-basics{grid-template-columns:repeat(3,minmax(0,1fr))}.free-editor .free-fields,.free-editor .free-features{container-type:inline-size;display:flex;flex-direction:column;gap:8px}.free-editor-section-header{display:flex;align-items:center;justify-content:space-between}.free-field-row{display:grid;grid-template-columns:minmax(120px,.8fr) minmax(180px,1.5fr) auto auto;gap:8px;padding:8px 10px;border-left:3px solid #b89564;background:#fffaf2}.free-feature-row{display:grid;grid-template-columns:minmax(0,1.3fr) minmax(0,1.3fr) auto;gap:8px;padding:9px 10px;border:1px solid #d8cec0;background:#fffaf2}.free-feature-description{grid-column:1/-1}.free-row-action{height:34px;min-height:34px;align-self:end}.free-empty-row{margin:0;padding:10px;color:#776b61;font-size:13px;text-align:center;border:1px dashed #cfc3b4;background:#fbf7f0}@container(max-width:440px){.free-field-row,.free-feature-row{grid-template-columns:repeat(2,minmax(0,1fr))}.free-row-action{width:100%}.free-feature-description{grid-column:1/-1}}@media(max-width:760px){.free-editor .free-basics{grid-template-columns:minmax(0,1fr)}}`}</style>
    ...
    <section className="free-features"><header className="free-editor-section-header"><h3>自由特性</h3><button type="button" onClick={() => onValue("内容", [...blocks, { ...emptyFeature }])}>＋ 新增</button></header>{blocks.length ? blocks.map((block, index) => <article className="free-feature-row" key={index}><EditorInput label="名称" value={block.名称} onChange={(value) => updateFeature(index, "名称", value)} /><EditorInput label="原文" value={block.原文} onChange={(value) => updateFeature(index, "原文", value)} /><ArrayItemActions item={block} index={index} count={blocks.length} onClear={() => onValue("内容", blocks.map((item, row) => row === index ? { ...emptyFeature } : item))} onDelete={() => onValue("内容", blocks.filter((_, row) => row !== index))} onMove={(offset) => onValue("内容", moveArrayItem(blocks, index, offset))} /><div className="free-feature-description"><EditorTextarea label="描述" value={block.描述} onChange={(value) => updateFeature(index, "描述", value)} /></div></article>) : <p className="free-empty-row">暂无自由特性</p>}</section>
  </div>;
}
export const freeAuthoring: TemplateAuthoringCapability = { templateId: "自由", templateVersion: "1.0.1", Editor: FreeAuthoringEditor };
