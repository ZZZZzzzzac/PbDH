...
function FreeCard({ data, presentation, portrait, attribution }: {
  ...
  }, [presentation.fixedRatio]);

  return <div
    className={`free-card-frame${presentation.fixedRatio ? "" : " is-fluid"}`}
    style={{ height: `${presentation.fixedRatio ? 88 : nativeHeight * freeScale}px` }}
  ><article ref={cardRef} className={["free-card", `is-${presentation.mode}`, presentation.fixedRatio ? "" : "is-fluid"].filter(Boolean).join(" ")} data-renderer-revision="free-card-r1">
