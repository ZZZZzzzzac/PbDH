...
import {
  weaponTemplate,
  type WeaponData,
} from "../../../core/weapon/1.0.1/capability.ts";
...
function WeaponCardFrame({
  ...
  useLayoutEffect(() => {
    ...
    const updateHeight = () => setNativeHeight(splitFixed ? fixedWeaponNativeHeight + (image?.offsetHeight ?? 0) : card.offsetHeight);
  ...
  return <div
    ...
    style={{ height: `${fixedRatio ? 88 : nativeHeight * weaponScale}px`, "--split-fixed-native-height": `${nativeHeight}px` } as CSSProperties}
...
> = {
  ...
  render({ data, presentation, assets, attribution }) {
    ...
    const cardClass = ["weapon-card", `is-${mode}`, fixedSurface ? "" : "is-fluid", splitFixed ? "has-fixed-base" : "", portrait ? "has-portrait" : ""]
      .filter(Boolean).join(" ");
    const header = <header className="weapon-header">
      ...
      {data.简介 ? <RestrictedMarkdown className="weapon-flavor" inline value={data.简介} /> : null}
    </header>;
    return <WeaponCardFrame fixedRatio={fixedSurface} splitFixed={splitFixed} imageKey={portrait}>{(cardRef) => <article ref={cardRef} className={cardClass} data-renderer-revision="weapon-card-r3" data-presentation-mode={mode}>
      {mode === "image" ? <div className="weapon-art is-image-only">
        ...
        <div className="weapon-body">
          ...
          <TextFitContainer className="weapon-description" contentKey={`${data.特性名称}\0${data.特性原文 ?? ""}\0${data.特性描述}`} enabled={fixedSurface} minFontSizePx={11} maxFontSizePx={15} cssVariable="--weapon-content-font-size">
