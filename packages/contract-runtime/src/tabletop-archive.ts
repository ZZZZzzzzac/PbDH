...
const ROOT_PATH = "tabletop.json";
const ASSET_PATH = /^assets\/([0-9a-f]{64})\.webp$/;
const MAX_ARCHIVE_BYTES = 16 * 1024 * 1024;
const MAX_ENTRIES = 1024;
const MAX_EXPANDED_BYTES = 64 * 1024 * 1024;

export type TabletopDocumentCandidate = {
  document: TabletopDocument;
  media: Map<string, Uint8Array>;
};

export type TabletopDocumentCandidateValidator = (
  document: TabletopDocument,
  media: TabletopMedia,
) => Promise<ContractDiagnostic[]>;

export type PbtabLoadResult = {
  candidate: TabletopDocumentCandidate | null;
...
export async function loadPbtab(
  bytes: Uint8Array,
  validate: TabletopDocumentCandidateValidator,
): Promise<PbtabLoadResult> {
  if (bytes.byteLength > MAX_ARCHIVE_BYTES) {
    return { candidate: null, diagnostics: [diagnostic(
      "tabletop-document.archive.bytes-exceeded",
      "",
  ...
  try {
    ...
    return { candidate: null, diagnostics: [diagnostic("tabletop-document.archive.zip.invalid", "")] };
  }
  const entries = Object.entries(files);
  if (entries.length > MAX_ENTRIES) {
    return { candidate: null, diagnostics: [diagnostic(
      "tabletop-document.archive.entry-count-exceeded",
      "",
  ...
  if (expandedBytes > MAX_EXPANDED_BYTES) {
    return { candidate: null, diagnostics: [diagnostic(
      "tabletop-document.archive.expanded-bytes-exceeded",
      "",
  ...
  if (!root) {
    return { candidate: null, diagnostics: [diagnostic("tabletop-document.archive.root.missing", `/${ROOT_PATH}`)] };
  }

  let document: TabletopDocument;
  try {
    const parsed = JSON.parse(strFromU8(root)) as unknown;
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error();
    document = parsed as TabletopDocument;
  } catch {
    return { candidate: null, diagnostics: [diagnostic("tabletop-document.archive.root.invalid-json", `/${ROOT_PATH}`)] };
  }
  ...
  for (const [path, fileBytes] of entries) {
    ...
    if (!match) {
      archiveDiagnostics.push(diagnostic("tabletop-document.archive.file.unknown", `/${path}`, { path }));
      continue;
    }
