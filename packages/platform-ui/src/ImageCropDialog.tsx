...
type DragState =
  ...
  | { mode: "resize"; corner: ImageCropCorner };
...
export interface ImageCropDialogProps {
  ...
  onCancel(): void;
  onConfirm(selection: ImageCropSelection): void;
}

export function ImageCropDialog({ file, label, working, processingError, fixedAspectRatio, onCancel, onConfirm }: ImageCropDialogProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const imageRef = useRef<HTMLImageElement | null>(null);
  const dragRef = useRef<DragState | null>(null);
  const layoutRef = useRef<ImageCropLayout | null>(null);
  const [layout, setLayout] = useState<ImageCropLayout | null>(null);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [aspectRatioChoice, setAspectRatioChoice] = useState<AspectRatioChoice>(fixedAspectRatio ? "card" : "free");
  const selectedAspectRatio = fixedAspectRatio ?? resolveAspectRatio(aspectRatioChoice, imageRef.current);
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      ...
      try {
        setLayout(initialCropLayout(image.naturalWidth, image.naturalHeight, fixedAspectRatio));
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: globalThis.WheelEvent) => {
      ...
      const x = (event.clientX - bounds.left) * cropViewportWidth / Math.max(1, bounds.width);
      const y = (event.clientY - bounds.top) * cropViewportHeight / Math.max(1, bounds.height);
      setLayout(zoomCrop(currentLayout, x, y, event.deltaY));
    };
      ...
      <section className="player-image-crop-dialog" role="dialog" aria-modal="true" aria-label={`裁剪${label}`}>
        <header className="player-image-crop-header">
          <div className="player-image-crop-heading">
            <div><p>图片处理</p><h2>裁剪{label}</h2></div>
          ...
          <div className="player-image-crop-actions">
            <label>
              ...
              <select
                aria-label="裁剪比例"
                value={fixedAspectRatio ? "card" : aspectRatioChoice}
                disabled={!layout || working || Boolean(fixedAspectRatio)}
                onChange={(event) => {
...
function drawCropper(canvas: HTMLCanvasElement, image: HTMLImageElement, layout: ImageCropLayout) {
...
function cropCorners(layout: ImageCropLayout): Array<[number, number]> {
  return [
