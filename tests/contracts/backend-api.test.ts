...
const contract = JSON.parse(readFileSync(
  "contracts/backend-api/1.2.0/openapi.json",
  "utf8",
)) as Record<string, any>;
const cases = JSON.parse(readFileSync(
  "contracts/conformance/backend-api/1.2.0/cases.json",
  "utf8",
)) as { operations: Array<[string, string, string]>; storageDataSchemas: string[] };

describe("Backend API 1.2.0 consumer conformance", () => {
  test("requires data byte counts in storage summaries and entries", () => {
    for (const name of cases.storageDataSchemas) {
      expect(contract.components.schemas[name].properties.dataBytes.type).toBe("integer");
      expect(contract.components.schemas[name].required).toContain("dataBytes");
    }
  });
